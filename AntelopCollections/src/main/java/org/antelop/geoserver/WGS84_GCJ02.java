...
public class WGS84_GCJ02
{
    ...
    {
        double ret = -100.0 + 2.0 * x + 3.0 * y + 0.2 * y * y + 0.1 * x * y + 0.2 * Math
            .sqrt(Math.abs(x));
    ...
    public double transformLon(double x, double y)
    {
        ...
        ret += (150.0 * Math.sin(x / 12.0 * pi) + 300.0 * Math
            .sin(x / 30.0 * pi)) * 2.0 / 3.0;
        return ret;
    }
    ...
    public Map<String, Double> gcj2wgs(double lon, double lat)
    {
        ...
        double lontitude = lon - (((Double) transform(lon, lat).get("lon"))
            .doubleValue() - lon);
    ...
    {
        System.out.println("gcj02-->84:  将坐标(119.307494,26.070824),转为: (" + new WGS84_GCJ02().gcj2wgs(119.307494,26.070824).get("lon") + "," +  new WGS84_GCJ02().gcj2wgs(119.307494,26.070824).get("lat") + ")" );
        System.out.println("84-->gcj02:  将坐标(119.30267602929797,26.07390544968644),转为: (" +  new WGS84_GCJ02().transform(119.30267602929797,26.07390544968644).get("lon") + "," +   new WGS84_GCJ02().transform(119.30267602929797,26.07390544968644).get("lat") + ")" );
    }
